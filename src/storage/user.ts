import { IUser, ILocalUser } from "../types/interface";
...
export const get = async (userId: string, channelId: string): Promise<ILocalUser> => {
  updateFromLocal()
  // update()
  ...
  const fetched = await api(`users/getUserInfoByUserId?channelId=${channelId}&userId=${userId}`, {
    method: 'get'
  })
  const userInfo = {
